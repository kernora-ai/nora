...
| Agent | Claw | Install |
|-------|------|---------|
| Claude Code | Built-in | Installed by `install.sh` — 6 hooks + MCP server |
...
### Claude Code — 6 hooks + MCP server

| Hook | What Nora does | Blocks? |
|------|---------------|---------|
| `SessionStart` | Injects project patterns at session start; recovers context after compaction | No |
| `UserPromptSubmit` | Searches past sessions, injects relevant context | No |
| `PreToolUse` | Validates tool input against known anti-patterns | **Yes** (exit 2) |
| `PostToolUse` | Checks output for known error signatures, logs metrics | No |
| `PreCompact` | Saves critical context before compaction destroys it | No |
| `Stop` | Captures transcript, sends to daemon for analysis | No |

Nora also runs as an **MCP server** — Claude Code can call `nora_search`, `nora_patterns`, `nora_bugs` and more as tools.
